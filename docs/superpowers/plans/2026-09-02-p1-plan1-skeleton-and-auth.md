...
## 後續任務：速率限制 + Argon2 不阻塞 event loop（兩件事必須一起做）

**這兩件事看起來無關，但綁在一起 —— 只做其中一件會讓情況變糟。**

**量測到的事實（對著跑起來的容器打）：**

| 項目 | 數字 |
|---|---|
| 登入嘗試速率（序列） | 約 16–17 次/秒（中位數 59.8ms/次） |
| 登入嘗試速率（20 並發） | 約 14.9 次/秒 |
| 登入嘗試速率（50 並發） | 約 16.1 次/秒 |
| `/api/health` 平時延遲 | 2.40ms |
| `/api/health` 在 20 並發登入下 | 平均 67.27ms，最高 378.73ms |

**兩個發現：**

1. **Argon2 卡住 event loop。** `verify_password` 是同步的 CPU 密集呼叫，寫在
   `async def` 路由裡。一個客戶端狂試登入，就會讓 tailnet 上所有人的所有請求變慢。
   標準解法是 `asyncio.to_thread(...)` 或 `run_in_threadpool(...)`，一個呼叫點兩行。

2. **但那個缺陷剛好是一道保護。** 並發完全不會提升攻擊者的吞吐量（20 並發跟
   50 並發一樣快），因為 event loop 把 Argon2 序列化了 —— 這構成一個粗糙的
   **全域**速率上限，約 140 萬次/天，不管攻擊者開幾條連線。

   **所以單獨修掉阻塞，等於拿掉這個意外的保護，而速率限制還沒補上。**

**現在的曝險：** 16 次/秒之下，前一萬名的常見密碼清單約 10 分鐘跑完，前十萬名約
1.7 小時。密碼下限是 8 字元、沒有複雜度也沒有外洩清單檢查，所以「弱但合法」的密碼
是真的會被猜到的。強密碼則完全無所謂 —— 暴力破解不可行。

**為什麼現在不急：** 能連到 API 的人本來就能自己註冊帳號（規格 §12 的決定），
所以他有比暴力破解別人帳號更便宜的路。**但 §12 記載的那個殘留風險一旦成真
（API 跑到 tailnet 外），這就立刻變成緊急的** —— 開放註冊 + 沒有 email 驗證 +
沒有速率限制，正是掃描機器人在找的組合。

**建議做法：** `slowapi` 搭記憶體儲存（單一實例不需要 Redis），套在
`POST /api/auth/login` 與 `POST /api/auth/register`，每個來源 IP 約 5–10 次/分鐘，
429 走既有的錯誤信封。**同一個 task 一起做掉 `asyncio.to_thread`。**
排在任何 P4 的網路曝露變更**之前**，不要延到 P4 裡面。

---

**另一件相關但不需要處理的事：** `/api/auth/register` 對已註冊的 email 會在幾毫秒內
回 `409 EMAIL_TAKEN`（存在性檢查發生在 `hash_password` 之前）。所以 Task 12 在登入端
做的防列舉保護，在實務上是**局部的** —— 任何 tailnet 成員都能透過註冊端點快速列舉。
這是註冊 UX 的常見取捨（總得告訴使用者「這個 email 已經被用了」），不是 bug，
但要知道這個保護沒有覆蓋全部。
...
而且沒有重用偵測。**這點已實測確認：換發之後，舊的那張仍然可以繼續用。**所以 14 天不是上限，是一個**只要裝置持續使用就永遠不會關上的
...
- **P2：`users` 該加一個「停用」欄位。** 現在要切斷單一問題帳號，只能直接用 `psql`
  刪資料列，或換 `JWT_SECRET`（會把所有人一起登出）。P2 本來就要為管理員角色動
  `users` 相關的東西，那時候一起加一個 `disabled_at`（`get_current_user` 跟 `login`
  都要檢查）最便宜；之後再補就是額外一次 migration 加上回頭改每一條認證路徑。
...
- **P3：refresh token 放在回應 body 裡，前端要怎麼存必須先決定。** 現在兩個 token
  ...
  拿走 access token 跟那張 14 天、而且會自動續期的 refresh token。
  **在前端動工之前決定**（例如 refresh token 改用 `httpOnly` cookie）；
  等前端寫完再改，成本高得多。
