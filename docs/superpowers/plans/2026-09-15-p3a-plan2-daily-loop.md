...
> **⚠️ 實測結果：第一個突變一開始【沒有】變紅（計畫二的第四個缺陷，
> 而且是這份計畫最重要的一個）。**
>
> 根因：共用的 `QueryClient` 沒設 `staleTime`，預設是 0 —— 任何 query
> 一 fetch 完就立刻「過期」。而 React Router 切換路由時會把 `Today`
> **整個 unmount／remount**，TanStack Query 在 remount 時只要資料是過期的
> 就會自動重取，**跟有沒有呼叫 `invalidateQueries` 完全無關**。
>
> 所以「記一餐 → 回到總覽 → 數字變了」是**因為 remount 而成立的**，
> 不是因為失效。把 `invalidateQueries(dailyStats)` 整行刪掉，這條 E2E
> 依然綠。
>
> **也就是說那條路徑同時沒有被 108 個單元測試守到、也沒有被這條 E2E
> 守到** —— 而這條 E2E 存在的唯一理由就是守它。
>
> 交接文件 §6 第 12 種是「**紅燈**的原因跟被測性質無關」。這一個是它的
> 鏡像：**綠燈的原因跟被測性質無關** —— 測試會通過，不管那個性質
> 成不成立。而鏡像那一側更難發現，因為沒有人會去追問一個綠燈為什麼綠。
>
> **修法是給 `QueryClient` 一個非零的 `staleTime`（用 60 秒）。**
> remount 時資料還新鮮就不會自動重取，那條路徑的正確性才真的只剩
> `invalidateQueries` 在守。這是一個 production 行為的改動，不是測試專用
> 的 hack —— 每次切換路由都重取本來就是浪費，只是它剛好也在掩護一個缺陷。
>
> **代價：這條 E2E 的鑑別力現在依賴 `staleTime` 大於測試執行時間**
> （60 秒 vs 約 4 秒，margin 夠）。那是一個跨檔案、靠註解維持的不變量 ——
> `src/api/queries.ts` 與 `e2e/daily-loop.spec.ts` **兩邊都要寫**，
> 而且要明說「改那個值之前先把 `invalidateQueries` 拿掉跑一次」。
