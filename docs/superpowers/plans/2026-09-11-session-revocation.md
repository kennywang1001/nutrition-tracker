...
| 3b | `rotate_session` 拿掉 `await _lock_user_sessions(...)` | 新的並行測試 | ⚠️ **實測結果與規格 §3.2 的 59/60 不符，需要下一位審查者留意**（Task 4 二輪品質審查）。`tests/test_sessions_concurrency.py` 用 `asyncio.gather` 讓兩個真實連線自然競速，**在這台機器上（Windows + Docker Desktop）跑了 25 次全部通過，一次都沒有變紅**。加碼追蹤（獨立診斷腳本，逐句記錄時間戳與 `pg_stat_activity`）發現：這台機器上兩個連線的自然交錯**穩定地**落在另一個方向——REPLAY 的 `_revoke_family` bulk UPDATE 總是先鎖到列 B，導致 ROTATE-B 自己的條件式 UPDATE 被擋下、乾淨失敗，從未走到 INSERT C；而規格描述的壞交錯（T_rotate 的 UPDATE+INSERT+COMMIT 全部搶在 revoke 的 bulk UPDATE 抵達列 B 之前完成）在自然時序下沒有出現。**這不代表洞是假的**：另外寫了一個用原始 asyncpg、逐句手動控制交錯順序的獨立腳本（不在測試套件內），強制製造規格描述的那個交錯，結果是無鎖 5/5 留下活票（`live_tokens_after: 1`）、有鎖 5/5 乾淨（`live_tokens_after: 0`，且確認第二個呼叫真的卡在 advisory lock 上，從未進到會產生交錯的任何一行）——**機制與修法都用強制交錯的方式定案為真**，但 `test_sessions_concurrency.py` 依賴的自然競速在這台機器上不足以穩定重現，可能跟 Docker Desktop 在 Windows 上的網路層時序特性有關。留著這條測試（它至少在「有鎖」時持續驗證沒有回歸），但不能把它在這裡「通過」當成「鎖有效」的證據——真正的證據是強制交錯的那組結果。下一位審查者如果要在 CI 或另一台機器上重跑，請留意這個環境差異 |
