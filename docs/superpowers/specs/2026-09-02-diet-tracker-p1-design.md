...
1. **tailnet 的範圍通常比單一 app 寬。** 例如為了讓家人用 Plex 而把他們加進
   tailnet，他們就也能在這裡註冊。
2. **更嚴重的情境：API 根本跑到 tailnet 外面去。** Tailscale Funnel、P4 部署時
   一個 port forward 設錯，都會讓「開放註冊 + 沒有 email 驗證 + 沒有速率限制」
   直接暴露在網際網路上。

真的需要收緊時，最便宜的做法是一個環境變數的共用邀請碼（沒設就等於關閉註冊），
不需要改資料庫、不需要審核流程。

> **⚠️ P4 必須處理：這個決定假設「連得到 API」等於「已經在 tailnet 裡」，
> 而那個假設目前沒有被任何東西強制執行。**
>
> `docker-compose.yml` 現在是 `"8000:8000"`，綁在 `0.0.0.0` —— 主機的所有介面。
> P4 要讓那個假設成真：綁到 Tailscale 的介面，或用 `tailscale serve` 擋在前面。
> 在那之前，這個決定的前提是不成立的。
