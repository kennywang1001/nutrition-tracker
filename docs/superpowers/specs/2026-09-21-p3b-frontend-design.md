...
> 都會給出正確答案。**時區把它遮住了。**
>
> 這代表：一條只在本機跑、只在 CI（UTC）跑的**行為測試**不會紅。這份規格
> 原本的結論是「所以測試必須在一個 UTC 以西的時區下執行」——
> **那個結論被實作推翻了**，見下。
>
...
>
> **而掃描本身要先把註解剝掉。** 第一版沒剝，於是這個模組寫不出自己禁止
> 什麼（docstring 裡的 `getDate()` 例句會讓掃描紅），而同一個名字寫在測試
> 檔的註解裡卻沒事 —— 一個分不出「程式碼在呼叫它」與「註解在解釋不要呼叫
> 它」的守衛。剝註解又開了新的失敗面（剝太多會讓掃描什麼都沒看到而變綠，
> 實測確認過那個假綠燈），所以另外補了兩條測試釘住它。詳見計畫一 Task 2。
